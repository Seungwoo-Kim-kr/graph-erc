...
Graph-based methods have been applied to dialogue to capture relational
structures that sequential models miss. DialogueGCN~\cite{ghosal2019dialoguegcn}
constructs heterogeneous graphs with speaker-specific and temporal edge
types and applies relational graph convolution for ERC.
The Knowledge-Enriched Transformer~\cite{zhong2019ket} augments
transformer self-attention with a context-aware affective graph over
ConceptNet, dynamically injecting commonsense knowledge at each
utterance representation. COSMIC~\cite{ghosal2020cosmic} integrates five
types of commonsense relations from ATOMIC into a flow model that
propagates knowledge through the dialogue graph.
DAG-ERC~\cite{shen2021dag} addresses long-range dependency by encoding
utterance relationships as a directed acyclic graph, explicitly
distinguishing forward and backward contextual influences.
SKIER~\cite{li2023skier} combines symbolic discourse-structure knowledge
with a commonsense knowledge graph subgraph retrieved for each utterance,
achieving state-of-the-art results on MELD, IEMOCAP, and EmoryNLP.
...
among retrieval-augmented methods, outperforming Text-only RAG by 0.011
and BM25 RAG by 0.038. Full-context LLM achieves a comparable macro F1
(0.491) by providing the complete dialogue history, though at the cost
of near-zero faithfulness as shown in Section~\ref{sec:faith-results}.
LLM-only achieves the highest accuracy (0.618) overall but this does
not correspond to faithful reasoning. Text-only RAG records the best
worst-class F1 (0.333) among all methods, indicating that recency-based
context selection helps minority classes more than semantic retrieval.
